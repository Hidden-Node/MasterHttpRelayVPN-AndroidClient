...
private fun parseProfileConfigJsonForImport(fileName: String, rawContent: String): ImportedProfileDraft? {
    ...
    val advanced = JsonObject().apply {
        ...
        addProperty("relay_timeout", obj.get("relay_timeout")?.asInt ?: 25)
        addProperty("tls_connect_timeout", obj.get("tls_connect_timeout")?.asInt ?: 15)
        addProperty("tcp_connect_timeout", obj.get("tcp_connect_timeout")?.asInt ?: 10)
        addProperty("max_response_body_bytes", obj.get("max_response_body_bytes")?.asLong ?: 209715200L)
        addProperty("parallel_relay", obj.get("parallel_relay")?.asInt ?: 1)
        addProperty("chunked_download_extensions", jsonArrayToLines(obj.get("chunked_download_extensions")))
        addProperty("chunked_download_min_size", obj.get("chunked_download_min_size")?.asLong ?: 5242880L)
        addProperty("chunked_download_chunk_size", obj.get("chunked_download_chunk_size")?.asLong ?: 524288L)
        addProperty("chunked_download_max_parallel", obj.get("chunked_download_max_parallel")?.asInt ?: 8)
        addProperty("chunked_download_max_chunks", obj.get("chunked_download_max_chunks")?.asInt ?: 256)
        ...
        addProperty("youtube_via_relay", obj.get("youtube_via_relay")?.asBoolean ?: false)
